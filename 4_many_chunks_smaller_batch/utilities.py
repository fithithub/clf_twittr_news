...
def break_in_chunks(system_message,tweets,model):
    ...
    <number of tweet> & <category> & <1-5 elements> could need up to 30 tokens, maybe more but these are special cases
    checked with https://platform.openai.com/tokenizer
    supposing "worst" case scenario:
    tokens_output = 30*n_tweets

    chunk = input + output = (system_message + T) + 30*number of tweets in T
    ...
    while(unfinished):
        for i,new_tweet in enumerate(list_tweets[last_i:],start=last_i):
            ...
            num_tokens = num_tokens_sytem + len(tokens) + 30*(n_tweets_chunk+1) # tokens until now + output from all previous tweets
            if(num_tokens >= 2000): # 4000 =approx 4096 == max tokens gpt3.5
                chunks.append(text)
                ...
                print("Tokens inside current chunk + system_message + output prediction: ",
                    len(encoding.encode(text)) + num_tokens_sytem + 30*n_tweets_chunk)
                text = "" 
                n_tweets_chunk = 0
                break
            else:
    ...
    print("Tokens inside last chunk + system_message + output prediction:", 
          len(encoding.encode(text))+ num_tokens_sytem + 30*(n_tweets_chunk+1))
